...
## Standard fields and column_order

Always use these standard names in `column_order` for the fixed fields:
- `date` — transaction date
- `number` — the column containing **primary document identifiers** (invoice numbers, credit
  note numbers, transaction numbers — the values used to match against accounting software).
  The PDF header might say "Reference", "Ref No.", "Doc No.", "Invoice No.", "Trans No." —
  map based on content, not header name.
- `due_date` — due/payment date (omit from column_order if not present)
- `reference` — secondary identifiers (PO numbers, customer codes) that are NOT the primary
  document number. If a column is clearly a cross-reference or document reference (e.g.
  "Cross Ref", "Doc Ref", "Your Ref"), map it to `reference`. Omit if not present.
