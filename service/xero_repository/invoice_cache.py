...
@dataclass
class InvoiceCachePayload:
    """Serialized invoice cache payload stored per tenant."""
...
class InvoiceCache:
    """Facade for loading and persisting invoice cache payloads."""

    def __init__(self, store: S3JSONResourceStore) -> None:
        self._store = store

    def load(self, tenant_id: str) -> Optional[InvoiceCachePayload]:
        ...
        return InvoiceCachePayload(
            invoices=[dict(item) for item in invoices],
            last_synced_at=last_synced_at,
            last_updated_utc=last_updated_utc,
        )

    def save(self, tenant_id: str, payload: InvoiceCachePayload) -> None:
