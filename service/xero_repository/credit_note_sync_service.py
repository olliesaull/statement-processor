...
from .credit_note_cache import CreditNoteCache, CreditNoteCachePayload
...
class CreditNoteSyncService:
    """Background synchronisation manager for tenant credit notes."""

    def __init__(
        self,
        cache: CreditNoteCache,
    ...
    def get_status(self, tenant_id: Optional[str]) -> ResourceSyncState:
        if not tenant_id:
            return ResourceSyncState(resource="credit_notes", status="empty")
        ...
        with self._status_lock:
            ...
            if state is not None:
                if state.status == "ready":
                    ...
                    if not payload:
                        logger.info("Credit note cache missing; resetting sync state", tenant_id=tenant_id)
                        empty_state = ResourceSyncState(resource="credit_notes", status="empty")
        ...
        if payload:
            derived = ResourceSyncState(
                ...
                status="ready",
                synced_count=len(payload.credit_notes),
                total_count=len(payload.credit_notes),
    ...
    def get_credit_notes(self, tenant_id: Optional[str]) -> List[Dict[str, Any]]:
        if not tenant_id:
            return []
        payload = self._cache.load(tenant_id)
        if not payload:
            return []
        return list(payload.credit_notes)
    ...
    def clear(self, tenant_id: Optional[str]) -> None:
        ...
        logger.info("Cleared credit note cache", tenant_id=tenant_id)
    ...
    def ensure_background_sync(self, tenant_id: Optional[str], *, force_full: bool = False) -> None:
        ...
        if not token:
            logger.error("Credit note sync skipped; OAuth token unavailable", tenant_id=tenant_id)
        ...
        logger.info("Queuing credit note sync", tenant_id=tenant_id, force_full=force_full, previous_status=state.status)
    ...
    def _update_status(self, tenant_id: str, **kwargs) -> None:
        with self._status_lock:
            current = self._status.get(tenant_id, ResourceSyncState(resource="credit_notes", status="empty"))
    ...
    def _run_sync(self, tenant_id: str, force_full: bool) -> None:
        ...
        if not token:
            logger.error("Credit note sync aborted; missing token snapshot", tenant_id=tenant_id)
        ...
        try:
            ...
            logger.exception("Credit note sync aborted; failed to build API client", tenant_id=tenant_id, error=str(exc))
        ...
        logger.info("Starting credit note sync", tenant_id=tenant_id, mode="full" if run_full else "delta", force_full=force_full)
        if run_full:
    ...
    def _perform_full_sync(self, tenant_id: str, api: AccountingApi) -> None:
        ...
        try:
            while True:
                ...
                for item in batch:
                    record, updated_at = self._serialize_credit_note(item)
                    key = record.get("credit_note_id") or record.get("number")
                    if not key:
                        continue
                    credit_notes[str(key)] = record
                    if updated_at and (not max_updated_utc or updated_at > max_updated_utc):
                        max_updated_utc = updated_at

                synced_count = len(credit_notes)
                self._update_status(
                    tenant_id,
                    status="syncing",
            ...
            ordered = sorted(credit_notes.values(), key=lambda cn: (cn.get("number") or "").casefold())
            last_synced_at = datetime.now(tz=timezone.utc).isoformat()
            payload = CreditNoteCachePayload(
                credit_notes=ordered,
            ...
            logger.info("Credit note sync completed", tenant_id=tenant_id, credit_notes=len(ordered), force_full=True)
        except Exception as exc:  # pragma: no cover - defensive logging
            logger.exception("Credit note sync failed", tenant_id=tenant_id, error=str(exc))
            self._update_status(tenant_id, status="error", error=str(exc))

    def _perform_delta_sync(self, tenant_id: str, payload: CreditNoteCachePayload, api: AccountingApi) -> None:
        ...
        if not since:
            logger.info("Credit note delta sync fallback to full; missing baseline", tenant_id=tenant_id)
            self._perform_full_sync(tenant_id, api)
            return
        ...
        for note in payload.credit_notes:
            key = note.get("credit_note_id") or note.get("number")
            if key:
                existing[str(key)] = dict(note)
        ...
        try:
            ...
            if updated_count == 0:
                ...
                logger.info("Credit note delta sync completed with no changes", tenant_id=tenant_id)
                return

            ordered = sorted(existing.values(), key=lambda cn: (cn.get("number") or "").casefold())
            last_synced_at = datetime.now(tz=timezone.utc).isoformat()
            last_updated_utc = (
            ...
            new_payload = CreditNoteCachePayload(
                credit_notes=ordered,
                last_synced_at=last_synced_at,
            ...
            logger.info(
                "Credit note delta sync completed",
                tenant_id=tenant_id,
                credit_notes=len(ordered),
                updates=updated_count,
                since=since.isoformat(),
            )
        except Exception as exc:  # pragma: no cover - defensive logging
            logger.exception("Credit note delta sync failed", tenant_id=tenant_id, error=str(exc))
            self._update_status(tenant_id, status="error", error=str(exc))

    def _serialize_credit_note(self, credit_note: Any) -> tuple[Dict[str, Any], Optional[datetime]]:
        contact_obj = getattr(credit_note, "contact", None)
        contact = (
            {
        ...
        total = getattr(credit_note, "total", None)
        remaining_credit = getattr(credit_note, "remaining_credit", None)

        record = {
            "credit_note_id": getattr(credit_note, "credit_note_id", None),
            "number": getattr(credit_note, "credit_note_number", None),
            "type": getattr(credit_note, "type", None),
            "status": getattr(credit_note, "status", None),
            "date": fmt_date(getattr(credit_note, "date", None)),
            "due_date": fmt_date(getattr(credit_note, "due_date", None)),
            "reference": getattr(credit_note, "reference", None),
            "subtotal": getattr(credit_note, "sub_total", None),
            "total_tax": getattr(credit_note, "total_tax", None),
            "total": remaining_credit if remaining_credit is not None else total,
            "amount_credited": getattr(credit_note, "amount_credited", None),
            "remaining_credit": remaining_credit,
            "contact": contact,
        }

        updated_at = parse_updated_datetime(getattr(credit_note, "updated_date_utc", None))
