...
def match_invoices_to_statement_items(
    items: list[StatementItemPayload], rows_by_header: list[dict[str, str]], item_number_header: str | None, invoices: list[XeroDocumentPayload]
) -> MatchedInvoiceMap:
    """
...
def _statement_items_by_number(items: list[StatementItemPayload], item_number_header: str) -> dict[str, StatementItemPayload]:
    """Build lookup of statement items keyed by their displayed invoice number."""
    stmt_by_number: dict[str, StatementItemPayload] = {}
    for item in items:
        raw = item.get("raw", {}) if isinstance(item, dict) else {}
...
def _record_exact_matches(stmt_by_number: dict[str, StatementItemPayload], invoices: list[XeroDocumentPayload]) -> tuple[MatchedInvoiceMap, set, set]:
...
def _candidate_invoices(invoices: list[XeroDocumentPayload], used_invoice_ids: set, used_invoice_numbers: set) -> list[tuple[str, XeroDocumentPayload, str]]:
    """Collect candidate invoices for substring matching."""
    candidates: list[tuple[str, XeroDocumentPayload, str]] = []
    for inv in invoices or []:
        inv_no = inv.get("number") if isinstance(inv, dict) else None
        if not inv_no:
            continue
...
def _missing_statement_numbers(rows_by_header: list[dict[str, str]], item_number_header: str, matched: MatchedInvoiceMap) -> list[str]:
...
def _candidate_hits(target_norm: str, candidates: list[tuple[str, XeroDocumentPayload, str]], used_invoice_ids: set, used_invoice_numbers: set) -> list[tuple[str, XeroDocumentPayload, int]]:
    """Collect candidate hits for a target invoice number."""
    hits: list[tuple[str, XeroDocumentPayload, int]] = []
...
def _mark_invoice_used(invoice_obj: XeroDocumentPayload, invoice_number: str, used_invoice_ids: set, used_invoice_numbers: set) -> None:
