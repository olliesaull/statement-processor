...
1. `try_acquire_sync` atomically claims the tenant via `UpdateItem` with a stale-heartbeat escape hatch (5 minutes). Rejection is a silent no-op when the worker retries its own claim in `sync_data`; the `/api/tenants/<id>/sync` and `/api/tenants/<id>/retry-sync` endpoints acquire the lock in the request thread first and surface rejection as 409 so the UI reflects the conflict immediately.
...
- **Sync button** (`FREE` / `LOADING` tenants): HTMX `POST /api/tenants/<id>/sync` synchronously acquires the sync lock in the request thread, then submits `sync_data(already_acquired=True)` to the background executor. The synchronous acquire means the returned fragment already reflects `TenantStatus=SYNCING` — without it the UI showed "Ready" for 2–4 seconds until the next 3 s poll. Returns 409 when another sync is in flight; see decision log 2026-04-23 item 1 (supersedes the 2026-04-20 entry that kept `/sync` fire-and-forget).
...
| `afk.js` | Tracks user activity (mouse/keyboard/scroll, 60 s inactivity threshold) and tab visibility, and toggles the `hx-disable` attribute on the polling panels (`#sync-progress-panel`, `#statement-reconcile-not-ready`) so HTMX stops polling when the tab is hidden or the user walks away. Uses JS event listeners instead of an `hx-trigger` bracket filter because the latter relies on `new Function()` which our CSP forbids (see decision log 2026-04-20). Replaces the pre-contacts-first-unlock `tenant-sync.js` module. |
...
| `tenant-card-local-time.js` | Rewrites server-rendered `<time>` elements inside the tenant cards from UTC to the user's locale using `Intl.DateTimeFormat`. Runs on `DOMContentLoaded` and on `htmx:beforeSwap` against the detached HTML string so the 3 s poll doesn't flicker UTC between ticks. JS-off users see the UTC fallback. See decision log entry 2026-04-23, item 4. |

`main.js`, `tenant-card-detail.js`, and `tenant-card-local-time.js` are the three files referenced directly in `base.html` via `<script type="module" src="...">`. The two `tenant-card-*` modules are listed separately so their `htmx:beforeSwap` listeners attach even when `main.js` is absent (progressive enhancement). The browser fetches `scroll-proxy.js` and `afk.js` automatically as ES module imports — no extra `<script>` tags are needed.
